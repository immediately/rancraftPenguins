...
public class EntityPenguinMac extends EntityPenguin
{
    ...
    public EntityPenguinMac(World par1World)
    {
        super(par1World);
    ...
    protected String getLivingSound()
    {
        if (isAngry())
        {
            return RanCraftPenguins.modID+":penguinMac.angry";
        }

        if (rand.nextInt(RanCraftPenguins.pengQuietInt) == 0) {
	        if(dataWatcher.getWatchableObjectFloat(18) < healthQuartile1) {
	    		return RanCraftPenguins.modID+":penguinMac.healthd";
	    	} else if(dataWatcher.getWatchableObjectFloat(18) < healthQuartile2) {
	    		return RanCraftPenguins.modID+":penguinMac.healthc";
	    	} else if(dataWatcher.getWatchableObjectFloat(18) < healthQuartile3) {
	    		return RanCraftPenguins.modID+":penguinMac.healthb";
	    	} else {
	    		return RanCraftPenguins.modID+":penguinMac.healtha";
	    	}
        } else {
        	return null;
        }
    ...
    {
        return RanCraftPenguins.modID+":penguinMac.hurt";
    }
    ...
    {
        return RanCraftPenguins.modID+":penguinMac.death";
    }
    ...
    public Item droppedFeather()
    {
		...
		if(i > 8){ // 60% black 30% white, 10% yellow
     ...
     */
    public EntityPenguinMac spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityPenguinMac var2 = new EntityPenguinMac(this.worldObj);
    ...
    public boolean canMateWith(EntityAnimal par1EntityAnimal)
    {
        ...
        else if (!(par1EntityAnimal instanceof EntityPenguinMac))
        {
            return false;
        }
        else
        {
            EntityPenguinMac var2 = (EntityPenguinMac)par1EntityAnimal;
